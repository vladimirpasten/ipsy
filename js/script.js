	...
	$document.ready(function () {
		...
		if (plugins.cdScheduleWrap.length) {
			...
			SchedulePlan.prototype.openModal = function (event) {
				...
				//alert(event.find('.event-id').text());
				//this.modalHeader.find('.event-id').val(event.find('.event-id').text());
				...
				if (mq == 'mobile') {
					...
					var eventTop = event.offset().top - $(window).scrollTop() - 300,
				...
				if (mq == 'mobile') {
					...
					setTimeout(function () {
						self.modal.css({
							...
							left: ((windowWidth / 2 - modalWidth / 2)-400) + 'px',
						});
